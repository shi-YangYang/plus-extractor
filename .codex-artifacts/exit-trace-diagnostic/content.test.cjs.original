...
test("content script manually applies a US proxy from pool 1 to the current page", async (t) => {
  ...
  assert.deepEqual(
    runtimeMessages
      .filter((message) => ["checkout-helper:set-proxy", "checkout-helper:test-proxy"].includes(message.type))
      .map((message) => message.type === "checkout-helper:set-proxy" ? `set:${message.phase}` : "test"),
    ["set:create", "test"]
  );
